...
class DestinationCollectionViewCell: UICollectionViewCell {
    ...
    override func layoutSubviews() {
        ...
        self.gradientView.frame = self.containerImageView.frame
        self.containerImageView.addSubview(gradientView)
        self.setupShadow()
    }
    ...
    private func configureStackView(rating: Int) {
        (0..<5).forEach { rate in
            ...
            imageView.image = UIImage(systemName: rate < rating ? "star.fill" : "star")
            self.stackViewRating.addArrangedSubview(imageView)
        }
    }
    ...
    func setupCell(destination: Destination) {
        self.destinationLabel.text = destination.name
        self.configureStackView(rating: destination.rating)
        self.labelTagButton.setTitle(destination.tag, for: .normal)
